...
class Vectorizer:
    ...
    def word2vec(self):
        ...
        vectors = [
            np.array([vectorizer[word] for word in tweet if word in model]).flatten() for tweet in
            tqdm(self.data, 'Vectorizing')
        ]
        if not self.max_len:
            self.max_len = np.max([len(vector) for vector in vectors])
        self.vectors = [
            np.array(vector.tolist() + [0 for _ in range(self.max_len - len(vector))]) for vector in
            tqdm(vectors, 'Finalizing')
        ]
        return self.vectors
    ...
    def BoW(self):
        ...
        vectors = [
            np.array([counts[mapper[word]] for word in tweet if word in mapper.keys()]).flatten() for tweet in
            tqdm(self.data, 'Vectorizing')
        ]
        if not self.max_len:
            self.max_len = np.max([len(vector) for vector in vectors])
        ...
        self.words_freq = sorted([[word, counts[mapper[word]]] for word in list(mapper.keys())], key=lambda x: x[1],
                                 reverse=True)
        return self.vectors
    ...
    def glove(self):
        ...
        vectorizer = model.wv
        vectors = [np.array([vectorizer[word] for word in tweet if word in model]).flatten() for tweet in
                   tqdm(self.data, 'Vectorizing')]
        self.vocab_length = len(model.wv.vocab)
        if not self.max_len:
            self.max_len = np.max([len(vector) for vector in vectors])
